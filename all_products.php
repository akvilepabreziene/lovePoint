<?php include 'header.php';?>

<div class="container my-5">
    <div class="row ">
        <div class="col-6 col-sm-3 mb-3">
            <div class="card zoom-img">
                <a href="product.php"><img src="pictures/product-5.jpg" class="card-img-top" alt="..."></a>
                <div class="card-body p-2">
                    <div class="row align-items-center">
                        <div class="col-12 col-sm-6">
                            <h5 class="card-title my-auto">Vitae aliquet</h5>
                        </div>
                        <div class="col">
                            <div class="row align-items-center">
                                <div class="col">
                                    <span class="products-price my-0">20 &euro;</span>
                                </div>
                                <div class="col">
                                    <a href="#"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"><path d="M0 0h24v24H0z" fill="none"/><path d="M17.21 9l-4.38-6.56c-.19-.28-.51-.42-.83-.42-.32 0-.64.14-.83.43L6.79 9H2c-.55 0-1 .45-1 1 0 .09.01.18.04.27l2.54 9.27c.23.84 1 1.46 1.92 1.46h13c.92 0 1.69-.62 1.93-1.46l2.54-9.27L23 10c0-.55-.45-1-1-1h-4.79zM9 9l3-4.4L15 9H9zm3 8c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2z"/></svg></a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-6 col-sm-3 mb-3">
            <div class="card zoom-img">
                <a href="product.php"><img src="pictures/1-4.jpg" class="card-img-top" alt="..."></a>
                <div class="card-body p-2">
                    <div class="row align-items-center">
                        <div class="col-12 col-sm-6">
                            <h5 class="card-title my-auto">Vitae aliquet</h5>
                        </div>
                        <div class="col">
                            <div class="row align-items-center">
                                <div class="col">
                                    <span class="products-price my-0">20 &euro;</span>
                                </div>
                                <div class="col">
                                    <a href="#"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"><path d="M0 0h24v24H0z" fill="none"/><path d="M17.21 9l-4.38-6.56c-.19-.28-.51-.42-.83-.42-.32 0-.64.14-.83.43L6.79 9H2c-.55 0-1 .45-1 1 0 .09.01.18.04.27l2.54 9.27c.23.84 1 1.46 1.92 1.46h13c.92 0 1.69-.62 1.93-1.46l2.54-9.27L23 10c0-.55-.45-1-1-1h-4.79zM9 9l3-4.4L15 9H9zm3 8c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2z"/></svg></a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-6 col-sm-3 mb-3">
            <div class="card zoom-img">
                <a href="product.php"><img src="pictures/product-2.jpg" class="card-img-top" alt="..."></a>
                <div class="card-body p-2">
                    <div class="row align-items-center">
                        <div class="col-12 col-sm-6">
                            <h5 class="card-title my-auto">Vitae aliquet</h5>
                        </div>
                        <div class="col">
                            <div class="row align-items-center">
                                <div class="col">
                                    <span class="products-price my-0">20 &euro;</span>
                                </div>
                                <div class="col">
                                    <a href="#"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"><path d="M0 0h24v24H0z" fill="none"/><path d="M17.21 9l-4.38-6.56c-.19-.28-.51-.42-.83-.42-.32 0-.64.14-.83.43L6.79 9H2c-.55 0-1 .45-1 1 0 .09.01.18.04.27l2.54 9.27c.23.84 1 1.46 1.92 1.46h13c.92 0 1.69-.62 1.93-1.46l2.54-9.27L23 10c0-.55-.45-1-1-1h-4.79zM9 9l3-4.4L15 9H9zm3 8c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2z"/></svg></a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-6 col-sm-3 mb-3">
            <div class="card zoom-img">
                <a href="product.php"><img src="pictures/product-3.jpg" class="card-img-top" alt="..."></a>
                <div class="card-body p-2">
                    <div class="row align-items-center">
                        <div class="col-12 col-sm-6">
                            <h5 class="card-title my-auto">Vitae aliquet</h5>
                        </div>
                        <div class="col">
                            <div class="row align-items-center">
                                <div class="col">
                                    <span class="products-price my-0">20 &euro;</span>
                                </div>
                                <div class="col">
                                    <a href="#"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"><path d="M0 0h24v24H0z" fill="none"/><path d="M17.21 9l-4.38-6.56c-.19-.28-.51-.42-.83-.42-.32 0-.64.14-.83.43L6.79 9H2c-.55 0-1 .45-1 1 0 .09.01.18.04.27l2.54 9.27c.23.84 1 1.46 1.92 1.46h13c.92 0 1.69-.62 1.93-1.46l2.54-9.27L23 10c0-.55-.45-1-1-1h-4.79zM9 9l3-4.4L15 9H9zm3 8c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2z"/></svg></a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>


    </div>

    <div class="row mb-4">
    <div class="col-6 col-sm-3 mb-3">
            <div class="card zoom-img">
                <a href="product.php"><img src="pictures/product-4.jpg" class="card-img-top" alt="..."></a>
                <div class="card-body p-2">
                    <div class="row align-items-center">
                        <div class="col-12 col-sm-6">
                            <h5 class="card-title my-auto">Vitae aliquet</h5>
                        </div>
                        <div class="col">
                            <div class="row align-items-center">
                                <div class="col">
                                    <span class="products-price my-0">20 &euro;</span>
                                </div>
                                <div class="col">
                                    <a href="#"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"><path d="M0 0h24v24H0z" fill="none"/><path d="M17.21 9l-4.38-6.56c-.19-.28-.51-.42-.83-.42-.32 0-.64.14-.83.43L6.79 9H2c-.55 0-1 .45-1 1 0 .09.01.18.04.27l2.54 9.27c.23.84 1 1.46 1.92 1.46h13c.92 0 1.69-.62 1.93-1.46l2.54-9.27L23 10c0-.55-.45-1-1-1h-4.79zM9 9l3-4.4L15 9H9zm3 8c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2z"/></svg></a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-6 col-sm-3 mb-3">
            <div class="card zoom-img">
                <a href="product.php"><img src="pictures/4-4.jpg" class="card-img-top" alt="..."></a>
                <div class="card-body p-2">
                    <div class="row align-items-center">
                        <div class="cocol-12 col-sm-6">
                            <h5 class="card-title my-auto">Vitae aliquet</h5>
                        </div>
                        <div class="col">
                            <div class="row align-items-center">
                                <div class="col">
                                    <span class="products-price my-0">20 &euro;</span>
                                </div>
                                <div class="col">
                                    <a href="#"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"><path d="M0 0h24v24H0z" fill="none"/><path d="M17.21 9l-4.38-6.56c-.19-.28-.51-.42-.83-.42-.32 0-.64.14-.83.43L6.79 9H2c-.55 0-1 .45-1 1 0 .09.01.18.04.27l2.54 9.27c.23.84 1 1.46 1.92 1.46h13c.92 0 1.69-.62 1.93-1.46l2.54-9.27L23 10c0-.55-.45-1-1-1h-4.79zM9 9l3-4.4L15 9H9zm3 8c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2z"/></svg></a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-6 col-sm-3 mb-3">
            <div class="card zoom-img">
                <a href="product.php"><img src="pictures/3-4.jpg" class="card-img-top" alt="..."></a>
                <div class="card-body p-2">
                    <div class="row align-items-center">
                        <div class="col-12 col-sm-6">
                            <h5 class="card-title my-auto">Vitae aliquet</h5>
                        </div>
                        <div class="col">
                            <div class="row align-items-center">
                                <div class="col">
                                    <span class="products-price my-0">20 &euro;</span>
                                </div>
                                <div class="col">
                                    <a href="#"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"><path d="M0 0h24v24H0z" fill="none"/><path d="M17.21 9l-4.38-6.56c-.19-.28-.51-.42-.83-.42-.32 0-.64.14-.83.43L6.79 9H2c-.55 0-1 .45-1 1 0 .09.01.18.04.27l2.54 9.27c.23.84 1 1.46 1.92 1.46h13c.92 0 1.69-.62 1.93-1.46l2.54-9.27L23 10c0-.55-.45-1-1-1h-4.79zM9 9l3-4.4L15 9H9zm3 8c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2z"/></svg></a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-6 col-sm-3 mb-3">
            <div class="card zoom-img">
                <a href="product.php"><img src="pictures/2-4.jpg" class="card-img-top" alt="..."></a>
                <div class="card-body p-2">
                    <div class="row align-items-center">
                        <div class="col-12 col-sm-6">
                            <h5 class="card-title my-auto">Vitae aliquet</h5>
                        </div>
                        <div class="col">
                            <div class="row align-items-center">
                                <div class="col">
                                    <span class="products-price my-0">20 &euro;</span>
                                </div>
                                <div class="col">
                                    <a href="#"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"><path d="M0 0h24v24H0z" fill="none"/><path d="M17.21 9l-4.38-6.56c-.19-.28-.51-.42-.83-.42-.32 0-.64.14-.83.43L6.79 9H2c-.55 0-1 .45-1 1 0 .09.01.18.04.27l2.54 9.27c.23.84 1 1.46 1.92 1.46h13c.92 0 1.69-.62 1.93-1.46l2.54-9.27L23 10c0-.55-.45-1-1-1h-4.79zM9 9l3-4.4L15 9H9zm3 8c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2z"/></svg></a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>

<?php include 'footer.php';?>